...
class chemicalcomp {
    ...
    function addLI() {
        ...
        $sql = "select nxtnum from seqnum where tablename = 'chemical_composition_li' for update";
        $result = mysql_query($sql);
        if(!$result) {
           $sql = "rollback";
           $result = mysql_query($sql);
           die("Seqnum access failed for chemical composition line items..Please report to Sysadmin. " . mysql_error());
        }
        ...
        $sql = "INSERT INTO chemical_composition_li(recno,
                                                    constituents,
                                                    standard_min,
                                                    standard_max,
                                                    supplier_min,
    ...
    //echo "chem" . $sql;
        ...
        if(!$result) {
           $sql = "rollback";
           $result = mysql_query($sql);
           die("Insert to chemical composition line items didn't work..Please report to Sysadmin. " . mysql_error());
        }
         $sql = "update seqnum set nxtnum = $objid where tablename = 'chemical_composition_li'";
   // echo "$sql";
        $result = mysql_query($sql);
        // Test to make sure query worked
        if(!$result) {
    ...
    function updateLI($recnum) {
        ...
        $sql = "update chemical_composition_li set
                               lineno = $lineno,
                              ...
                              remarks = $remarks,
                              link2testreport = $link2testreport

                        where recno = $recnum";
    //echo $sql;
           $result = mysql_query($sql);
           // Test to make sure query worked
           if(!$result) die("Update to chemical composition didn't work..Please report to Sysadmin. " . mysql_error());
...
//echo $sql;
     ...
     function deleteLI($inplirecnum) {
        ...
        $sql = "delete from chemical_composition_li where recno = $lirecnum";
        // echo "$sql";
        $result = mysql_query($sql);
