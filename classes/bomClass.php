...
class bom {
    ...
    function setassywonum($assywonum)
    {
      $this->assywonum = $assywonum;
    }
    ...
    function addBOM()
    {
      ...
      $sql = "select * from bom where bomnum = $bomnum and bom_revnum= $bomrevnum and 
      ...
      {
        $sql = "INSERT INTO	bom 
                  (recnum, bomnum, bom_issue, crn, assy_partnum, title, issue,cos_no,cos_iss,drg_no,bom_revnum,eng_app,eng_app_by,eng_app_date,status,create_date,
                    partiss,siteid,assywonum,link2assywo)
                VALUES
		              ($objid, $bomnum, $bom_issue, $crn, $assy_part,$title, $issue,$cos_no,$cos_iss,$drg_no,
                  $bomrevnum,$eng_app,$eng_app_by,$eng_app_date,$status,now(),
                  $partiss,$siteid,$assywonum,$link2assywo)";
        $result = mysql_query($sql);
    ...
    function getBOMs($bomrecnum)
    {
        ...
        $sql = "select recnum,bomnum, bom_issue, crn, assy_partnum,
                       title,issue, cos_no,cos_iss,drg_no,bom_revnum ,eng_app,eng_app_by,
                       eng_app_date,status,create_date,partiss,assywonum,link2assywo
                FROM bom
                where recnum=$bomrecnum";
       //echo $sql;
        $result = mysql_query($sql);
...
//function to get only BOMs where bomnum=partnum
  ...
  function getassy_crn()
       ...
       $sql = " select  bom_subassy.item_no as itn,bom_subassy.partnum,bom_subassy.partname, bom_subassy.partiss,'Assembly',
          ...
          select  bom_mfg.item_no as itn,bom_mfg.partnum,bom_mfg.partname, bom_mfg.partiss,'Non Assembly',
               ...
               select  bom_bo.item_no as itn,bom_bo.partnum,bom_bo.descr, bom_bo.partiss,'Bought Out','',bom_bo.qpa,'',''
                ...
                UNION
               select  bom_co.item_no as itn,bom_co.partnum,bom_co.descr,'','Consummables','',bom_co.qpa ,'',bom_co.spec
            ...
            function getbom4assyWopartDetails($bomnum,$crn_num,$bomiss)
    {
       ...
       $siteid = $_SESSION['siteid'];
       $siteval = "b.siteid = '".$siteid."'";
       $sql = "select  bom_mfg.item_no as itn,bom_mfg.partnum,bom_mfg.partname, bom_mfg.partiss,'Non Assembly',
       bom_mfg.crn,bom_mfg.qpa,bom_mfg.crn_type,''
                   from bom b,bom_mfg_items bom_mfg
               ...
               select  bom_co.item_no as itn,'',bom_co.descr,'','Consume','',bom_co.qpa,'',bom_co.spec
                   from bom b,bom_consume bom_co
                    where bom_co.link2bom=b.recnum and b.crn='$crn_num' and b.status ='Active' and b.bom_revnum='$bomiss' and $siteval order by itn
...
//echo $sql;
     ...
     function addbomnotes($bomrecnum,$bomnotes)
  {
    ...
    function getassywo4bom()
    {
      ...
      $sql = "select recnum,assy_wonum,crn,drgno,drgiss,cosno,assy_type,
                     assypartiss,assypartnum,status 
              from assy_wo
              where (assy_type = 'Assembly' || assy_type = 'Kit') and
                    status = 'Pending' and $siteval
              order by recnum";
      // echo "$sql"; exit;
      $result = mysql_query($sql);
      return $result;
