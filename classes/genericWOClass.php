...
class genericWO{
  ...
  function gettype() {
   ...
   function setpart6 ($reqpart6) {
    ...
    function addgenericWO() {
//        $newlogin = new userlogin;
//        $newlogin->dbconnect();
        //$sql = "start transaction";
       // $result = mysql_query($sql);
        $sql = "select nxtnum from seqnum where tablename = 'generic_wo' for update";
        $result = mysql_query($sql);
        if(!$result) {
           $sql = "rollback";
           $result = mysql_query($sql);
           die("seqnum access failed for genericWO  ..Please report to Sysadmin. " . mysql_error());
        }
	 ...
	 $qty10=$this->qty10;
        $type="'" . $this->type . "'";
        $sql = "INSERT INTO generic_wo(recnum,
	string1,
	string2,
	string3,
	string4,
...
//echo "$sql";
        ...
        if(!$result) {
           $sql = "rollback";
           $result = mysql_query($sql);
           die("Insert to genericWO didn't work..Please report to Sysadmin. " . mysql_error());
        }
        $sql = "update seqnum set nxtnum = $objid where tablename = 'generic_wo'";
        $result = mysql_query($sql);
        if(!$result) {
           $sql = "rollback";
           $result = mysql_query($sql);
           die("seqnum insert query didn't work for genericWO.Please report to Sysadmin. " . mysql_error());
        }
        return $objid;
     }

    function UpdategenericWO($argrecnum) {
        $recnum = $argrecnum;
        $newlogin = new userlogin;
        $newlogin->dbconnect();
       // $sql = "start transaction";
...
//echo "$sql";
           ...
           if(!$result) die("Update to genericWO didn't work..Please report to Sysadmin. " . mysql_error());
        return $wosqladd;
     }
     ...
     function getGenInfo($inpworecnum) {
       ...
       //echo "worecnum:$worecnum<br>";
        $sql = "select w.wonum, w.wotype, c.name, w.po_num,w.quote_num, w.status, w.wo2type,
                       ...
                       w.reorder,w.condition,w.wo2wfconfig,b.bomnum,b.recnum,w.grnnum, w.qty, w.sonum
                    from work_order w
