...
function getPosort($cond,$argsort1,$argoffset,$arglimit)
{
       ...
       if($usertype != 'VEND')
       {
          $sql = "select wo.recnum,p.ponum, p.podate, p.podescr, c.name,
                         ...
                         pl.material_spec,pl.material_ref,pl.qty_recd,pl.order_qty,
                         pl.due_date1,pl.due_date2,p.creation_date,p.last_modified,p.created_by
      ...
      else
      {
          $sql = "select wo.recnum,p.ponum, p.podate, p.podescr, c.name,
                       ...
                       pl.material_spec,pl.material_ref,pl.qty_recd,pl.order_qty,
                       pl.due_date1,pl.due_date2,p.creation_date,p.last_modified,p.created_by
                  from company c,po_line_items pl,user u,contact cont,po p
                  left join work_order wo on p.wonum = wo.wonum
                  where $wcond and
                        c.recnum = p.link2vendor and
                        pl.link2po=p.recnum 
