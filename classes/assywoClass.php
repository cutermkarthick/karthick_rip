  ...
  function getwos4assywo($crn,$type,$crn_type)
  {
    ...
    if($type == 'Non Assembly')
    {
      if($crn_type == 'Untreated')
      {
        $sql = "select  w.wonum,w.qty,w.recnum,w.crn_num,
                ...
                left join dispatch d on d.recnum = dli.link2dispatch 
                where 
   ...
   function getwos4assycrnwo($crn)
   {
     ...
     $siteval = "w.siteid = '".$siteid."'";
      ...
      /*$sql = "select w.assy_wonum,
              ...
              where
                    w.crn='$crn' and 
                    w.comp_qty - (w.dispatch_qty +w.kit_qty) > 0  and
                    w.comp_qty > 0";*/
   ...
   function getwoqty4custpo($ponum,$crnnum,$line_num) 
   {
      ...
      $sql = "select sum(wo.assyqty)
              from  assy_wo wo
				      where wo.ponum='$ponum' and
					          wo.crn='$crnnum' and
					          wo.`status` != 'Cancelled' and
                    (case when (wo.cust_po_line_num != '' && wo.cust_po_line_num is not null) then wo.cust_po_line_num = '$line_num' else true end)
              group by wo.ponum";
	    // echo $sql;
      $result = mysql_query($sql);
      if(!$result) {
    ...
    function getrejqty4custpo($ponum,$crnnum,$line_num) 
    {
      ...
      $sql = "select sum(wps.rej),sum(wps.ret)
              from assy_part_status wps, assy_wo wo
				      where wo.recnum=wps.link2assywo  and
					          wo.`status` != 'Cancelled' and
				            (wps.stage = 'final' or wps.stage = 'Final'	or wps.stage = 'FINAL' or wps.stage = 'fi' or
                    wps.stage = 'FI' or wps.stage = 'Fi') and
                    wo.ponum='$ponum' and wo.crn='$crnnum' and
				            (case when (wo.cust_po_line_num != '' && wo.cust_po_line_num is not null) then wo.cust_po_line_num = '$line_num' else true end)
              group by wo.ponum ";
      // echo $sql; 
      $result = mysql_query($sql);
    ...
    function getpos($cim_refnum,$partnum,$woclassif) 
    {
      ...
      $sql = "select ar.cust_ponum as ponum, ar.po_date, arli.qty,
                     c.name,c.recnum,arli.crn, '','','',
					    ...
					    from assy_review_li arli, company c, assy_review ar
					    where ar.customer = c.recnum and
     ...
     function getassywoqty4grn() {
      ...
      function getallgrns4assy($partnum)
      {
        ...
        $sql1 =  "select  g.recnum, g.grnnum,g.batch_num, sum(gli.qty_to_make) as qtm,
                          g.crn,g.raw_mat_type,g.raw_mat_spec,gli.partnum ,g.grntype,
                          gli.expdate
                  from grn g, grn_li gli
		...
		function getallgrns4cons($type)
      {
        ...
        $siteval = "g.siteid = '".$siteid."'";
        $sql1 = "select  g.recnum, g.grnnum,g.batch_num, sum(gli.qty_to_make) as qtm,
                         g.crn,g.raw_mat_type,g.raw_mat_spec,gli.partnum,
                         gli.expdate,g.cimponum
                       from grn g, grn_li gli
                       where
...
function getgrnli($grnrecnum) {
      ...
      function getbom_assyWo_partDetails($bomnum)
    {
       ...
       $sql = "select  bom_subassy.item_no as itn,bom_subassy.partnum,bom_subassy.partname, bom_subassy.partiss,'Sub Assembly',
                   ...
                   UNION
                 select  bom_mfg.item_no as itn,bom_mfg.partnum,bom_mfg.partname, bom_mfg.partiss,'Untreated',
  ...
  public function GetBOIRmpoNum4AssyWo($partnum, $type)
  {
    $newlogin = new userlogin;
    $newlogin->dbconnect();

    $sql = "select p.recnum,p.ponum,p.podate,p.type,
                   li.material_spec,li.crn,li.rate,li.line_num
            from po p, po_line_items li
            where p.recnum = li.link2po and
                  li.material_spec ='$partnum' and
                  p.type = '$type' and
                  p.status = 'Open' and
                  li.status = 'Open' ";
    // echo "$sql <br>";
  ...
  public function getNotes4assywo($link2wo)
  {

    $sql = "select n.create_date, n.wonotes, u.userid
            from assywo_notes n, user u, assy_wo w
            where n.link2wo = w.recnum and
                  n.link2user = u.recnum and
                  w.recnum = $link2wo order by n.recnum";
            // echo $sql;
    $result = mysql_query($sql);
    return $result;
  ...
  public function AddNotes4Assywo($link2wo, $notes)
  {
    $newlogin = new userlogin;
    $newlogin->dbconnect();
    ...
    $notes = "'" . $notes . "'";
    $sql = "INSERT INTO assywo_notes (wonotes,link2wo, link2user,create_date )
            VALUES ($notes, $link2wo, $userrecnum, curdate())";
    // echo "$sql <br>"; exit;
    $result = mysql_query($sql);
