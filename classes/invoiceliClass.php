...
class invoiceli {
    ...
    function addInvoiceli() {
        ...
        if(!$result) {
         ...
         $sql = "update seqnum set nxtnum = $objid where tablename = 'invoice_line_items'";
   // echo "$sql";
        $result = mysql_query($sql);
        // Test to make sure query worked
        if(!$result) {
    ...
    function deleteInvoiceli($inprecnum) {
        ...
        $newlogin->dbconnect();
         $recnum =$inprecnum;
        $sql = "delete from invoice_line_items where recnum = $recnum";
        // echo "$sql";
        $result = mysql_query($sql);
        if(!$result) die("Delete for Line Items failed...Please report to SysAdmin. " . mysql_error());
      }
      
       function deleteInvoiceliflag($inprecnum) {
        $newlogin = new userlogin;
        $newlogin->dbconnect();
         $recnum =$inprecnum;
        $sql = "delete from invoice_line_items where link2invoice = $recnum";
        // echo "$sql";
        $result = mysql_query($sql);
        if(!$result) die("Delete for Line Items failed...Please report to SysAdmin. " . mysql_error());
      }
