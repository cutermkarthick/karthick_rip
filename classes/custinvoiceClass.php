   ...
   function setfobcf($req_fobcf) {
    ...
    function addInvoice() {
       //echo "I am here";
        ...
        $sql = "select nxtnum from seqnum where tablename = 'cust_invoice' for update";
        //echo "$sql";
        $result = mysql_query($sql);
        if(!$result)
                     {
      ...
      $st = "'" . $this->st . "'";
       ...
       $cess2 = "'" . $this->cess2 . "'";		 
        ...
        $sql = "select * from cust_invoice where recnum = $objid";
       // echo $sql;
        $result = mysql_query($sql);
        ...
        $sql = "update seqnum set nxtnum = $objid where tablename = 'cust_invoice'";
        $result = mysql_query($sql);
        // Test to make sure query worked
        if(!$result)
                     {
     ...
     function customeraddress($invoicerecnum) {
       ...
       $sql = "select c.name, c.id,
               ...
               FROM cust_invoice i, company c, company c1
     ...
     function getinvoices($cond,$argoffset,$arglimit) {
        ...
        $limit = $arglimit;
         $sql = "select distinct il.link2invoice,i.*,c.name
                         from cust_invoice i,cust_invoice_line_items il ,
						          company c
                         where c.recnum = i.inv2customer  and
...
//        $row     = mysql_fetch_array($result, MYSQL_ASSOC);
  //      $numrows = $row['numrows'];
    ...
    function deleteInvoice($invoicerecnum) {
        $newlogin = new userlogin;
        $newlogin->dbconnect();
        $sql = "delete from cust_invoice where recnum = $invoicerecnum";
        $result = mysql_query($sql);
        if(!$result)
                     {
 ...
 function Invoicepayment($totaldue,$payment_amount,$invoicerecnum) {
   ...
   //echo "\n" . $sql;
     ...
     function getStatus1($invoicerecnum) {
        $newlogin = new userlogin;
        $newlogin->dbconnect();
        $sql = "select  status from cust_invoice where recnum = $invoicerecnum";
        $result = mysql_query($sql);
	...
	function getallcustpo($companyrecnum)
	{
	 $newlogin = new userlogin;
     $newlogin->dbconnect();
	...
	function getallcustpo4invoice($companyrecnum,$crn_num)
	{
	 ...
	 function getcustpoqty4invoice($crn_num,$companyrecnum,$po_num)
     {
        $newlogin = new userlogin;
        $newlogin->dbconnect();
        $sql = "select sum(li.qty)
               from cust_invoice_line_items li,cust_invoice l
                    where
                         li.crnnum='$crn_num' and
                         l.inv2customer=$companyrecnum and
                         li.link2invoice=l.recnum and
                         li.custpo_num='$po_num'
                         group by li.crnnum";
        //echo $sql;
        $result = mysql_query($sql);
        return $result;
      ...
      function getinvoices4po($custpo) {
        ...
        $newlogin->dbconnect();
         ...
         $sql = "select i.*,c.name,il.crnnum,il.cimpartnum,il.descr,il.type ,il.custpo_num
                         from cust_invoice i,cust_invoice_line_items il,
						          company c
                         where c.recnum = i.inv2customer and
                               il.link2invoice=i.recnum and
...
function copyinvoice($invoicerecnum)
{
	        $newlogin = new userlogin;
            $newlogin->dbconnect();
			$sql = "select nxtnum from  seqnum where tablename = 'invoice'";
            $result = mysql_query($sql);
		    if(!$result) {
                  die("Copy seqnum error.Please report to Sysadmin. " . mysql_error());
             }
		   $myrow = mysql_fetch_row($result);
           $objid4invoice = $myrow[0] + 1;

			$sql = "select nxtnum from  seqnum where tablename = 'invoice_line_items'";
            $result = mysql_query($sql);
		    if(!$result) {
                  die("Copy seqnum error.Please report to Sysadmin. " . mysql_error());
             }
		   $myrow = mysql_fetch_row($result);
           $objid4invoiceli = $myrow[0] + 1; 
		    $sql = "select * from  invoice where recnum = $invoicerecnum";
            $result = mysql_query($sql);
		    if(!$result) {
                  die("Copy Invoice error.Please report to Sysadmin. " . mysql_error());
             }

            $num_rows = mysql_num_rows($result);
            if ($num_rows > 0)
	        {
				  die ("Invoice $invoicerecnum already existing for this Customs Invoice");
	         }
            ...
            $sql = "INSERT INTO
              ...
              cess2						
                    ...
                    from cust_invoice where recnum = $invoicerecnum
                            ";

         $result = mysql_query($sql);
	     if(!$result)
	     {
			  echo "Insert failed for Copy for invoice $invoicerecnum ....Please report to sysadmin" . mysql_errno();
		      $sql = "rollback";
		      $result = mysql_query($sql);
			  exit;
    	 }
		         $sql = "INSERT INTO invoice_line_items (
						...
						select 
                               ...
                               polinenum,
							   schpo
						from cust_invoice_line_items 
						where link2invoice = $invoicerecnum;
                 ";
       //  echo $sql;
        $result = mysql_query($sql);
        // Test to make sure query worked
        if(!$result) {
           $sql = "rollback";
           $result = mysql_query($sql);
           die("Insert for Copy LI didn't work..Please report to Sysadmin");
        }
        $sql = "update seqnum set nxtnum = $objid4invoice where tablename = 'invoice'";
        //echo $sql;
        $result = mysql_query($sql);
        // Test to make sure query worked
        if(!$result)
                     {
                     //header("Location:errorMessage.php?validate=Inv4");
                     die("Seqnum insert for invoice  didn't work..Please report to Sysadmin. " . mysql_error());
                     }
        $sql = "update seqnum set nxtnum = $objid4invoiceli where tablename = 'invoice_line_items'";
        //echo $sql;
        $result = mysql_query($sql);
        // Test to make sure query worked
        if(!$result)
...
} // End Customs  invoice class definition
