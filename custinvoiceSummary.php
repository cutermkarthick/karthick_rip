...
$_SESSION['pagename'] = 'custinvoicesummary';
$page = "Invoice: Cust Invoice";
...
<html>
<head>
<title>Customs Invoice Summary</title>
</head>
<body leftmargin="0"topmargin="0" marginwidth="0">
<form action='custinvoiceSummary.php' method='post' enctype='multipart/form-data'>
<?php
	include('header.html');
?>
...
<tr>
<td bgcolor="#FFFFFF"><span class="labeltext"><b>Customs Invoice #</b>
...
<td bgcolor="#FFFFFF" colspan=2><input type="text" name="final_crn" size=10 value="<?php echo $final_crnmatch ?>" onkeypress="javascript: return checkenter(event)">
</td>
</tr>
...
<input type="button" class="stdbtn btn_blue" style="float:right;padding:2px;margin-right:5px;" onClick="location.href='custinvoiceEntry.php'" value="New"> 
 												</span></h2>
										      </table>
                                      ...
                                      <th class="head0"><span class="tabletext"><b>Amount</b></th>
                                     	...
                                     	<th class="head0"><span class="tabletext"><b>FOB/C&F/DAP</b></th>
													...
													$result = $newinvoice->getinvoices($cond,$offset,$rowsPerPage);
                                                         ...
                                                         //if($prev_inv != $myrow["invnum"] )
                                                         //{
                                                         //$prev_inv = $myrow["invnum"];
	     												 printf('<tr><td bgcolor="#FFFFFF"><span class="tabletext"><a href="custinvoiceDetails.php?invoicerecnum=%s&inv2customer=%s"><b>%s</b></td>
                                                                        <td bgcolor="#FFFFFF"><span class="tabletext">%s</td>
                                                                        <td bgcolor="#FFFFFF"><span class="tabletext">%s</td>
                          												<td bgcolor="#FFFFFF"><span class="tabletext">%s</td>
                                     	                                <td bgcolor="#FFFFFF" align="right"><span class="tabletext">%s %.2f</td>
																		<td bgcolor="#FFFFFF" align="right"><span class="tabletext">%s %.2f</td>
                         											 	<td align="center" bgcolor="#FFFFFF"><span class="tabletext">%s</td></tr>',
		     									     		               $myrow["recnum"],$myrow["name"],$myrow["invnum"],
                       													   wordwrap($myrow["name"],20,'<br>',true),
                       													   $idate,
                       													   $ddate,
																		   $myrow["currency"],
                          												   $myrow["total"],
...
{
    $page = $pageNum - 1;
    $prev = " <a href=\"custinvoiceSummary.php?page=$page&totpages=$totpages&final_relno=$finalrel_match&
	final_crn=$final_crnmatch&ddate1=$ddate1_match&ddate2=$ddate2_match&final_partnum=$final_partnummatch\">[Prev]</a> ";

    $first = " <a href=\"custinvoiceSummary.php?page=1&totpages=$totpages&final_relno=$finalrel_match&final_wo=$final_womatch&
...
{
    $page = $pageNum + 1;
    $next = " <a href=\"custinvoiceSummary.php?page=$page&totpages=$totpages&final_relno=$finalrel_match&
	final_crn=$final_crnmatch&ddate1=$ddate1_match&ddate2=$ddate2_match&final_partnum=$final_partnummatch\">[Next]</a> ";

    $last = " <a href=\"custinvoiceSummary.php?page=$totpages&totpages=$totpages&final_relno=$finalrel_match&
