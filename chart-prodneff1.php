...
$result = $newreport->gettime4prodn_eff($mcname,$dcond);
 ...
 while ($myrow = mysql_fetch_row($result)) {
      //echo 'inside whlie';
            ...
            {
             $crns[0]= $myrow[1];
             $esttimes[0]= ($myrow[4]+$myrow[7]);
             $actualtimes[0] = ($myrow[5]+$myrow[6]+$rej_time);
             $lossorgains[0]  =  (($myrow[4]+$myrow[7])-(($myrow[5]+$myrow[6]+$rej_time)));
            }
            else if($crns[$counter]==$myrow[1])
            {
             $esttimes[$counter] += ($myrow[4]+$myrow[7]);
             $actualtimes[$counter] += ($myrow[5]+$myrow[6]+$rej_time);
             $lossorgains[$counter] +=  (($myrow[4]+$myrow[7])-(($myrow[5]+$myrow[6]+$rej_time)));
            }
            else{
             $counter++;
...
//
$g = new graph();
$g->title( 'Efficiency for Machine:'.$mcname.'  From:'.$from_date.' To:'.$to_date, '{font-size:18px; color: #bcd6ff; margin:10px; background-color: #5E83BF; padding: 5px 15px 5px 15px;}' );
