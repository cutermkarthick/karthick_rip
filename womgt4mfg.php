...
// Filename: womgt4mfg.php                 =
// Copyright (C) FluentSoft Inc.               =
// Contact bmandyam@fluentsoft.com             =
// Revision: v1.0 OMS                          =
// Coded by Jerry George
// For adding and deleting Wos for mfg_orders
//==============================================
...
<body leftmargin="0"topmargin="0" marginwidth="0">
    <form action="processmfg.php?mfgrecnum=<?php echo "$mfgrecnum";?>" method='post' enctype='multipart/form-data'>
...
<table width=100% border=0 cellpadding=0 cellspacing=0  >
	...
	<tr>
	<td>
<input type=hidden name=mfgsubmit value="<?php echo "$add";?>">
<input type=hidden name=submit value="<?php echo "$add";?>">
<?php $newdisplay->dispLinks($mfgrecnum); ?>
<?php
if($add =="LinkWO")
{
?>
...
<?php
	if ($add=="LinkWO")
	 {
		   $result =$newmfg->addwo4mfg($cond,$mfgrecnum,$offset, $rowsPerPage);
		  $result1 =$newmfg->getAddwocount4mfg($cond,$mfgrecnum,$offset, $rowsPerPage);
		  $row     = mysql_fetch_array($result1, MYSQL_ASSOC); 
		  $maxrecno = $row['maxrecno']; 
	}
	else if($add =="UnlinkWO")
	{
	  	   $result = $newmfg->getwo4mfg($cond,$mfgrecnum,$offset, $rowsPerPage); 
		  $result1 =$newmfg->getwo4mfgcnt($cond,$mfgrecnum,$offset, $rowsPerPage);
		  $row     = mysql_fetch_array($result1, MYSQL_ASSOC); 
		  $maxrecno = $row['maxrecno']; 
	}
                $flag = 0;
