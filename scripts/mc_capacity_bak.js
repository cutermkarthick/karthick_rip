...
{  
    ...
    if (document.forms[0].units.value =='select' )
    {
         errmsg += 'Please Select units \n';
    }
...
function fetch_cap_req(inparg)
{
 ...
 if (document.forms[0].mc_series.value == 'select' )
    ...
    if (document.forms[0].pl_month.value == '' || document.forms[0].pl_month.value == 'select')
    {
         errmsg += 'Please select Month \n';		
    }
	 if (document.forms[0].pl_year.value == '' || document.forms[0].pl_year.value == 'select')
    {
	...
	while (flag == 0)
	{
		...
		if((prev_mc == document.getElementById('mc_name'+i).value))
		{
	     ...
	     //    document.getElementById('balance_mc_hrs'+i).value=Math.abs(balance_mc_hrs);
		    ...
		    var meridian =start_time.substr(start_time.length-2).toLowerCase();
		    var hrs    = start_time.substring(0, start_time.indexOf(' '));
	        ...
	        for(se = 0; se < 24 ; se++) 
	        {
			        ...
			        if(meridian == "pm" || meridian == "on")
			        {
			        	
			        	// console.log("inside if ");
		        		ed_sel = parseInt(end_time) + parseInt(12);

			        	
		        		// console.log('select val inside if' + ed_sel);
			        }
			        else if(meridian == "am" || meridian == "id")
			        {	
			        	// console.log("inside else ");
			        	ed_sel = parseInt(end_time);

			        	// console.log('select val endtime' + end_time);
			        }			        
                    ...
                    else if(se == 12)
                    {
                   	...
                   	// console.log("se val  " + se + " -- " + i);
                   	// console.log("select val  " + ed_sel + " --  " + i);
			...
			// var convert_days = hours/6000;
   			...
   			if (avail_units == "strokes") {
   				...
   				per_hr = stores_per_shift/shift_hr;
                total_hrs = hours/per_hr;
				...
				alert("hours "+hours);
   				var hours1 = hours%24;	
				alert("hours1 "+hours1);
				var days = parseInt(req_crn_hrs/24);
                alert("days "+days)
...
function calbal_qty_select_date(i)
{
    ...
    {		
		...
		if((prev_mc == document.getElementById('mc_name'+i).value)  )
		{
		    ...
		    req_rt_perunit=  (document.getElementById('req_rt_perunit'+i).value);	 
		    blank=  (document.getElementById('blank'+i).value);
		    blank_hrs = req_rt_perunit / blank;
