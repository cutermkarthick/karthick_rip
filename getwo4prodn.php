...
<html>
<head>
    <title>All my WO Nos</title>
</head>
<body onload=self.focus()>

<form action='getwo4prodn.php' method='post' enctype='multipart/form-data'>
<?
$result2 = $newoperator->getcurrenttime($mcname,$entdate,$shift);
...
?>
<input type="hidden"  id="total_prev_mins"  name="total_prev_mins" size=3 value="<?echo $total_prev_mins;?>">
...
?>
    ...
    <tr bgcolor="#FFFFFF"><td bgcolor="#FFFFFF" width=5%><input type="radio" name="crn"   value="<?php echo $myrow[6]."|".$myrow[5]."|".$myrow[1]."|".$myrow[4]."|".$myrow[7]."|".$myrow[13]?>"></td>
...
function SubmitCIM(etype){
 var flag=0;
 var user_input;
 var total_prev_mins=document.forms[0].total_prev_mins.value;
//alert(user_input);
//alert(ctype);
...
//alert(user_input);
//alert(total_prev_mins);
window.opener.Setwo_crn(user_input,etype,'<?php echo $flag?>',total_prev_mins);
self.close();
}
