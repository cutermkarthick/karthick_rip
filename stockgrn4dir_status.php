...
// Compute the total cost
     if ($flag == 0)
     {
        ...
        $total_cost_dollar = 0;
        $total_cost_null = 0;
        $result1 = $newreport->get_stockbygrn4totalcost($cond);
        while ($myrow = mysql_fetch_row($result1)) {
            ...
            if($currency == '$')
            {
              $total_cost_dollar += ($balance*$rmprice);
            }
            else if($currency == 'Rs')
            {
              $total_cost_rupee += ($balance*$rmprice);
            }
            else if($currency == '')
            {
              $total_cost_null += ($balance*$rmprice);
            }

         } 
       }
       echo "<tr><td colspan=11 align=\"right\" bgcolor=\"#A0CFEC\"><span class=\"labeltext\">Total Cost(Rs) : Rs. $total_cost_rupee  Total Cost($) : $ $total_cost_dollar  Total Cost(Null) : $total_cost_null</td></tr>";
*/
// End of total compute cost
	    ...
	    if($myrow[1] != '0000-00-00' && $myrow[1] != '' && $myrow[1] != 'NULL')
            ...
            $balance = 0;
            $balance = $myrow[4] + $myrow[8] - $myrow[7];
            $result4rmprice = $newreport->getrmprice($myrow[5]);
            $myrow4rmprice = mysql_fetch_row($result4rmprice);
...
<tr>
<td bgcolor="#FFFFFF" colspan=10 align="right"><span class="heading"><b>Total(Rs)</b></td>
<?php

printf('<td bgcolor="#FFFFFF" colspan=1 align="right"><span class="tabletext">%s %.2f</td>','Rs',$total_balance_cost_rupee);
?>
<tr>
<td bgcolor="#FFFFFF" colspan=10 align="right"><span class="heading"><b>Total($)</b></td>
<?php
printf('<td bgcolor="#FFFFFF" colspan=1 align="right"><span class="tabletext">%s %.2f</td>','$',$total_balance_cost_dollar);
?>
<tr>
<td bgcolor="#FFFFFF" colspan=10 align="right"><span class="heading"><b>Total(Null)</b></td>
<?php
printf('<td bgcolor="#FFFFFF" colspan=1 align="right"><span class="tabletext">%s %.2f</td>','',$total_balance_cost_null);
?>
</td></tr>
</table>
</td>
...
{
    $page = $pageNum - 1;
    $prev = " <a href=\"stockgrn4dir_status.php?page=$page&totpages=$totpages&scomp=$company_match&swonum=$wonum_match&grn=$grn_match&crn=$crn_match&sdate1=$date1_match&sdate2=$date2_match&flag=1&tcr=$total_cost_rupee&tcd=$total_cost_dollar&tcn=$total_cost_null\">[Prev]</a> ";

    $first = " <a href=\"stockgrn4dir_status.php?page=1&totpages=$totpages&scomp=$company_match&swonum=$wonum_match&grn=$grn_match&crn=$crn_match&sdate1=$date1_match&sdate2=$date2_match&flag=1&tcr=$total_cost_rupee&tcd=$total_cost_dollar&tcn=$total_cost_null\">[First Page]</a> ";
}
else
{
...
{
    $page = $pageNum + 1;
  $next = " <a href=\"stockgrn4dir_status.php?page=$page&totpages=$totpages&scomp=$company_match&swonum=$wonum_match&grn=$grn_match&crn=$crn_match&sdate1=$date1_match&sdate2=$date2_match&flag=1&tcr=$total_cost_rupee&tcd=$total_cost_dollar&tcn=$total_cost_null\">[Next]</a> ";

  $last = " <a href=\"stockgrn4dir_status.php?page=$totpages&totpages=$totpages&scomp=$company_match&swonum=$wonum_match&grn=$grn_match&crn=$crn_match&sdate1=$date1_match&sdate2=$date2_match&flag=1&tcr=$total_cost_rupee&tcd=$total_cost_dollar&tcn=$total_cost_null\">[Last Page]</a> ";
}
else
{
