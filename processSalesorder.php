...
$submit_type = $_REQUEST['subtype'];
$salesorderrecnum  = $_REQUEST['mysorecnum'] ;
//  echo "<br>myrecnum is $salesorderrecnum";
//echo "Department is $department and pagename is $pagename and submit_type is $submit_type";
...
{
   $salesorderrecnum  = $_REQUEST['mysorecnum'] ;
   //echo "<br>I am inside editsoooo recnum is $salesorderrecnum";
   $so2customer = $_REQUEST['companyrecnum'];
   //echo "<br>I am inside editsoooo companyrecnum is $companyrecnum";
   $description = addslashes($_REQUEST['description']);
   $phone = $_REQUEST['phone'];
   $email = $_REQUEST['email'];
	//$sales_order = $_REQUEST['sales_order'];
...
//------------------------------Review set-------------------------//
    //$newreview->setname($name);
	//$newreview->setordernum($ordernum);
    $newsalesorder->setnumofparts($numofparts);
    $newsalesorder->setattachment1($attachment1);
    $newsalesorder->setsource($source);
    $newsalesorder->setparts_class($parts_class);
    $newsalesorder->setresources($resources);
    $newsalesorder->setqualityreq($qualityreq);
    $newsalesorder->setsaliant($saliant);
    $newsalesorder->setaditional_resources($aditional_resources);
    $newsalesorder->setsubcontract($subcontract);
    $newsalesorder->setspecial_process($special_process);
    $newsalesorder->setdelivery_req($delivery_req);
    $newsalesorder->setperson($person);
    //$newreview->setquoterefnum($quoterefnum);
    $newsalesorder->setquotation($quotation);
    $newsalesorder->setdata_for_quote($data_for_quote);
    $newsalesorder->setdata_store($data_store);
    $newsalesorder->setpath($path);
    $newsalesorder->setquotation_det_store($quotation_det_store);
    $newsalesorder->setrisk_factors($risk_factors);
    $newsalesorder->setrequirements($requirements);
    $newsalesorder->setexplainrisk_factors($explain_risk_factors);
    $newsalesorder->setquote_sentby($quote_sentby);
    $newsalesorder->setquote_path($quote_path);
    $newsalesorder->setenquiry_path($enquiry_path);
    $newsalesorder->setdata_for_enquiry($data_for_enquiry);
    $newsalesorder->setorderfor($orderfor);
    $newsalesorder->setordertype($ordertype);
    $newsalesorder->setvalstatus($valstatus);
    $newsalesorder->setcreate_date($create_date);
    $newsalesorder->setcreated_by($created_by);
    $newsalesorder->setqa_approved($qa_approved);
    $newsalesorder->setengineering_approved($engineering_approved);
    $newsalesorder->setqa_app_by($qa_app);
    $newsalesorder->seteng_app_by($eng_app);


	if($review_notes != '')
	{
      $newsalesorder->addNotes($salesorderrecnum,$review_notes);
	}
...
if ($pagename == 'soDetails' || $pagename == 'viewsoDetails')
{
  $salesorderrecnum  = $_REQUEST['mysorecnum'] ;
  //$salesorderrecnum = $_REQUEST['salesorderrecnum'];
  $reviewrecnum  = $_REQUEST['reviewrecnum'] ;
...
else if ($pagename == 'soDetails' && $department == 'Sales')
{
    header("Location:so_review_details.php?salesorderrecnum=$salesorderrecnum");
}
else if ($pagename == 'editso')
{
   ...
   if($department == 'PPC'  ||  $department == 'PPC1'  || $department == 'PPC2' )
   {
	 header("Location:viewsalesorder.php");
   }
}
//else if ($pagename == 'newso' && $submit_type == 'save') {
...
else if($pagename == 'viewsoDetails' and ($department == 'PPC'  ||  $department == 'PPC1'  || $department == 'PPC2' ))
{
	 header("Location:viewsalesorder.php");
}
?>
